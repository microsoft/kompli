...
{
    auto result = BenchmarkInfo::FromMetadata("cis", "ubuntu", "20.04", "v1.0.0");
...
{
    auto result = BenchmarkInfo::FromMetadata("stig", "rhel", "9", "vV2R7");
    ASSERT_TRUE(result.HasValue()) << result.Error().message;
    EXPECT_EQ(result.Value().distribution, LinuxDistribution::RHEL);
    EXPECT_EQ(result.Value().version, std::string("9"));
...
TEST_F(BenchmarkInfoTest, Parse_AcceptsArbitraryFramework)
{
...
TEST_F(BenchmarkInfoTest, FromMetadata_AcceptsArbitraryFramework)
{
...
{
    auto result = BenchmarkInfo::FromMetadata("", "ubuntu", "20.04", "v1.0.0");
    ASSERT_FALSE(result.HasValue());
    ASSERT_EQ(result.Error().code, EINVAL);
    ASSERT_EQ(result.Error().message, "Benchmark framework must not be empty");
}

TEST_F(BenchmarkInfoTest, FromMetadata_RejectsUnknownDistribution)
{
    auto result = BenchmarkInfo::FromMetadata("cis", "unknown", "20.04", "v1.0.0");
    ASSERT_FALSE(result.HasValue());
    ASSERT_EQ(result.Error().code, EINVAL);
}

TEST_F(BenchmarkInfoTest, FromMetadata_AcceptsBenchmarkVersionWithoutVPrefix)
{
    auto result = BenchmarkInfo::FromMetadata("cis", "ubuntu", "20.04", "1.0.0");
    ASSERT_TRUE(result.HasValue()) << result.Error().message;
...
{
    auto result = BenchmarkInfo::FromMetadata("cis", "ubuntu", "20.04", "");
    ASSERT_FALSE(result.HasValue());
    ASSERT_EQ(result.Error().code, EINVAL);
}

...
void AppendPodSecurityFinding(std::ostringstream& findings, const std::string& namespaceName, const std::string& podName,
    const std::string& containerName, bool hostNetwork, bool hostIPC, bool hostPID, bool privileged, bool allowPrivilegeEscalation)
{
    findings << namespaceName << '\t' << podName << '\t' << containerName << "\thostNetwork=" << std::boolalpha << hostNetwork << "\thostIPC=" << hostIPC
             << "\thostPID=" << hostPID << "\tprivileged=" << privileged << "\tallowPrivilegeEscalation=" << allowPrivilegeEscalation << '\n';
}
...
Result<std::string> FilterPodSecurityOutput(const std::string& output)
{
    ...
    for (size_t itemIndex = 0; itemIndex < json_array_get_count(items); ++itemIndex)
    {
        ...
        {
            AppendPodSecurityFinding(findings, namespaceName, podName, "-", hostNetwork.Value(), hostIPC.Value(), hostPID.Value(), false, false);
        }
        ...
        for (size_t containerIndex = 0; containerIndex < json_array_get_count(containers); ++containerIndex)
        {
            ...
            {
                AppendPodSecurityFinding(findings, namespaceName, podName, containerValue, hostNetwork.Value(), hostIPC.Value(), hostPID.Value(),
                    privileged.Value(), allowPrivilegeEscalation.Value());
            }
        }
    }
    return findings.str();
